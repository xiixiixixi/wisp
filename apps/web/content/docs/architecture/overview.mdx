...
Wisp follows a **two-process architecture** powered by Tauri v2: a Rust backend for system operations and a React frontend for the UI.
...
User types search query
  → React calls TauriAPI.findFiles(query, root) for each volume root
    → transport('find_files', { pattern, searchPath })
      → The system search provider answers (mdfind on macOS)
      → Returns Vec<String> as JSON
    → React keeps the first 50 hits and displays them
```
