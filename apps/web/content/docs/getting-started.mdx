...
This guide will help you set up Wisp for development.
...
```bash
git clone https://github.com/wisp/wisp.git
cd wisp
```
...
- **`identifier`** — Unique app identifier (e.g., `com.wisp.app`)
...
├── pages/
│   ├── wisp.tsx      # Main file explorer page
...
├── lib/
│   ├── tauri-api.ts     # Tauri backend bridge (delegates to @wisp/sdk)
...
├── ollama_client.rs     # Shared Ollama HTTP client (agent chat + analysis)
