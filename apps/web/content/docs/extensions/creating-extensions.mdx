...
Wisp supports eight extension categories. This guide shows you how to build each one using the high-level registration APIs.
...
- Use `onActivate` to capture the `WispAPI` reference
...
Preview extensions render custom file previews. When a user selects a file, Wisp asks each preview extension if it can handle it.
...
Command.register({
  ...
  action: async (api) => {
    const state = (window as any).__wisp_state__;
...
```typescript
import { ContextMenu, type WispAPI } from '@wisp/extension-sdk';
...
```typescript
import { Tab, type WispAPI } from '@wisp/extension-sdk';
import { Button, Spinner, Card } from '@wisp/extension-sdk';
...
```typescript
import { BottomTab, type WispAPI } from '@wisp/extension-sdk';
import { Button, Panel } from '@wisp/extension-sdk';
...
Editor extensions provide custom file editing experiences. When a user opens a file for editing, Wisp checks registered editors for a match using `canEdit`.
...
Inside your extension's React components, you can use hooks to reactively read Wisp state:

```typescript
import { useCurrentPath, useSelectedFiles } from '@wisp/extension-sdk';
...
These hooks listen for `wisp-state-change` CustomEvents dispatched by the host, so they react instantly without polling.
...
```typescript
import { Button, Input, Card, Spinner, Panel } from '@wisp/extension-sdk';
...
```typescript
import { Extension, PanelExtension, registerExtension } from '@wisp/extension-sdk';
...
```typescript
import { createExtension, registerExtension } from '@wisp/extension-sdk';
