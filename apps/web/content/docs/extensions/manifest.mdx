...
Every Wisp extension declares its metadata, permissions, and contributions in the `wisp` field of `package.json`.
...
The `contributes` object declares what the extension provides to Wisp.
...
The `main` field in the top-level `package.json` (not inside `wisp`) specifies the compiled entry point:
