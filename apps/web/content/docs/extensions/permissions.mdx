...
Wisp uses a permission system to control what extensions can access. Extensions must declare their required permissions in the manifest. The backend validates all permissions during installation, and the frontend enforces them at runtime through the sandboxed WispAPI.

## How Permissions Work

1. **Declaration** — extensions list required permissions in `package.json` under `wisp.permissions`
2. **Validation** — the Rust backend checks that all permission strings are recognized
3. **Enforcement** — the `WispAPI` checks permissions before every method call
...
| Permission | String               | Access     | Description                         |
| ---------- | -------------------- | ---------- | ----------------------------------- |
| Settings   | `wisp:settings`   | Read/Write | Access Wisp application settings |
| Themes     | `wisp:themes`     | Modify     | Register and modify themes          |
| History    | `wisp:history`    | Read       | Access navigation and file history  |
| Extensions | `wisp:extensions` | Read       | Query other installed extensions    |
...
Extensions run in the same renderer process as Wisp — there is no iframe or Worker sandbox. The permission system is a **trust boundary**, not a security boundary. It prevents accidental misuse and makes extension capabilities transparent to users, but it does not protect against intentionally malicious code.
