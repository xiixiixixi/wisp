...
:::info
As of the two-layer SDK architecture, `TauriAPI` is a thin facade that delegates to `@wisp/sdk` service modules (16 modules wrapping Tauri `invoke()` calls) rather than making direct `invoke()` calls itself. The internal SDK lives at `wisp-sdk/` and the external extension SDK at `extension-sdk/`.
:::
