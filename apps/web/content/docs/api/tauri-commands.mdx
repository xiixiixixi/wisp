...
| Command                   | Parameters     | Returns               | Description              |
| ------------------------- | -------------- | --------------------- | ------------------------ |
| `find_files`      | `pattern, searchPath` | `Vec<String>`        | Filename search through the system provider |
| `search_in_files` | `pattern, searchPath` | `Vec<SearchMatch>`   | Scan file contents on demand |
| `grep_search`     | `query, searchPath`   | `Vec<GrepSearchMatch>` | Content search with context lines |
