...
const ChatFileView = ({ filePath }: ChatFileViewProps) => {
    ...
    return (
      <div className="flex h-full items-center justify-center bg-xp-bg">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-xp-text-muted">{tUi('interface.loadingChat')}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className="flex h-full flex-col bg-xp-bg"
      role="region"
      aria-label={tUi('interface.chatFileView')}
    >
      {/* ── Header ─────────────────────────────────────────────────────────── */}
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              aria-label={`Thinking mode: ${thinkingEnabled ? tUi('interface.enabled') : tUi('interface.disabled')}`}
              aria-pressed={thinkingEnabled}
            >
              {thinkingEnabled ? tUi('interface.thinkingOn') : tUi('interface.thinkingOff')}
            </button>
          </div>
          ...
          {contextFiles.length > 0 && (
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              <span className="text-xs text-xp-text-muted">{tUi('interface.contextLabel')}</span>
