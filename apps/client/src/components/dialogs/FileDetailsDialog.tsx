...
const FileDetailsDialog = ({
  ...
  return (
    ...
    >
      <div className="border-xp-border/60 mx-4 flex max-h-[80vh] w-full max-w-lg flex-col overflow-hidden rounded-md border bg-xp-surface/95 shadow-2xl shadow-black/30 duration-150 animate-in fade-in zoom-in-95">
        {/* Header */}
    ...
    <div className="border-xp-border/40 flex flex-shrink-0 border-b bg-xp-bg px-2 pt-1">
      {tabs.map((tab) => (
        <button
          ...
          className={`relative flex items-center gap-1.5 rounded-t-[2px] px-3 py-2 text-xs font-semibold transition-all ${
            activeTab === tab.id
        ...
        >
          ...
          {activeTab === tab.id && (
            <div className="absolute bottom-0 left-2 right-2 h-[2px] rounded-md bg-xp-lime" />
          )}
        </button>
      ))}
    </div>
            ...
            <div key={note.id} className="overflow-hidden rounded-md border border-xp-border">
              ...
              >
                <div className="flex min-w-0 items-center space-x-2">
                  ...
                  <span className="truncate text-sm font-semibold text-xp-text">{note.title}</span>
                </div>
            ...
            <>
              {activeAnnotations.length > 0 && (
                <div className="space-y-1.5">
                  {activeAnnotations.map((a) => (
                    <div
                      key={a.id}
                      className="group flex items-start space-x-2 rounded-md px-2 py-2 transition-colors hover:bg-xp-surface-light"
                    >
                      <button
                      ...
                      <button
                        ...
                        className="flex-shrink-0 rounded-md p-1 text-xp-text-muted opacity-0 transition-all hover:bg-xp-surface-light hover:text-xp-red group-hover:opacity-100"
                        title={t('performanceDashboard.ops.delete')}
                      >
                        <Trash2 className="h-3 w-3" />
                      </button>
              ...
              {resolvedAnnotations.length > 0 && (
                <div className="space-y-1">
                  <p className="pt-1 text-xs font-semibold uppercase tracking-wide text-xp-text-muted">
                    {t('messages.resolvedAnnotations', { count: resolvedAnnotations.length })}
                  </p>
                  {resolvedAnnotations.map((a) => (
                    <div
                      key={a.id}
                      className="group flex items-start space-x-2 rounded-md px-2 py-2 opacity-60 transition-colors hover:bg-xp-surface-light"
                    >
                      <button
                        onClick={() => handleToggleResolved(a.id)}
      ...
      <div className="flex-shrink-0 border-t border-xp-border px-4 py-3">
        <div className="flex items-center space-x-2">
          <input
            ...
            maxLength={500}
            className="flex-1 rounded-md border border-xp-border bg-xp-bg px-3 py-1.5 text-sm text-xp-text placeholder-xp-text-muted transition-colors focus:border-xp-blue focus:outline-none"
          />
          <button
            onClick={handleAdd}
            disabled={saving || !newText.trim()}
            className="flex items-center space-x-1 rounded-md bg-xp-blue px-2.5 py-1.5 text-sm font-semibold text-xp-on-accent transition-colors hover:bg-opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {saving ? (
    ...
    <>
      <div className="flex-1 space-y-2 overflow-y-auto px-4 py-3">
                ...
                >
                  <div className="relative">
                    <input
                      ...
                      className="w-full rounded-md border border-xp-border bg-xp-bg px-2 py-1.5 text-sm text-xp-text transition-colors focus:border-xp-blue focus:outline-none"
                    ...
                    {showKeySuggestions === field._localId &&
                      (() => {
                        ...
                        return (
                          <div className="absolute left-0 right-0 top-full z-10 mt-0.5 max-h-32 overflow-y-auto rounded-md border border-xp-border bg-xp-popover shadow-xl">
                            {suggestions.map((key) => (
                              <button
                                key={key}
                                onMouseDown={(e) => {
                  ...
                  <button
                    ...
                    className="rounded-md p-1.5 text-xp-text-muted transition-colors hover:bg-xp-surface-light hover:text-xp-red"
        ...
        <button
          ...
          className="flex w-full items-center justify-center space-x-1.5 rounded-md border border-dashed border-xp-border px-2 py-1.5 text-sm text-xp-blue transition-colors hover:bg-xp-surface-light"
        >
          <Plus className="h-3.5 w-3.5" />
      ...
      <div className="flex flex-shrink-0 items-center justify-end space-x-2 border-t border-xp-border px-4 py-3">
        ...
        <button
          ...
          className="flex items-center space-x-1.5 rounded-md bg-xp-blue px-3 py-1.5 text-sm font-semibold text-xp-on-accent transition-colors hover:bg-opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {saving ? (
            <>
              <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white border-t-transparent" />
