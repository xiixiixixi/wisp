  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-lg border-xp-border bg-xp-bg">
        ...
        {/* Extension Info */}
        <div className="flex items-start gap-3 rounded-md border border-xp-border bg-xp-surface p-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-xp-blue/20">
            <Package className="h-5 w-5 text-xp-blue" />
          </div>
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-semibold text-xp-text">
        ...
        <div>
          ...
          {permissions.length === 0 ? (
            <div className="rounded-md border border-xp-border bg-xp-surface p-3 text-center text-xs text-xp-text-muted">
              ...
              <div className="space-y-1.5">
                {sorted.map((perm) => {
                  ...
                  return (
                    <div
                      key={perm.id}
                      className={`flex items-start gap-2.5 rounded-md border p-2 ${colors.bg} ${colors.border}`}
                    >
                      <Icon className={`mt-0.5 h-4 w-4 shrink-0 ${colors.text}`} />
                      <div className="min-w-0">
                        <div className={`text-xs font-semibold ${colors.text}`}>{perm.label}</div>
