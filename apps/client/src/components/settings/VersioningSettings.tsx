...
const VersioningSettings = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {error && (
          <div className="mx-4 mb-2 rounded-[2px] border border-xp-red/20 bg-xp-red/10 px-3 py-2 text-xs text-xp-red">
            {error}
          </div>
        )}
        ...
        <div className="px-4 pb-2">
          <div className="flex items-center gap-2">
            <input
              ...
              className="placeholder:text-xp-text-secondary/50 h-9 flex-1 rounded-[2px] border border-xp-border bg-xp-bg px-3 text-sm text-xp-text focus:outline-none"
            />
            <button
              onClick={addDirectory}
              disabled={!newDir.trim()}
              className="flex h-9 items-center gap-1.5 rounded-[2px] bg-xp-accent/10 px-3 text-xs font-medium text-xp-accent transition-colors hover:bg-xp-accent/20 disabled:opacity-50"
            >
              <Plus size={14} />
        ...
        {config.enabled_dirs.length === 0 ? (
          ...
          <div className="space-y-4 px-4 pb-2">
            {config.enabled_dirs.map((dir) => (
              <div
                key={dir}
                className="border-xp-border/30 flex items-center justify-between gap-2 rounded-[2px] border bg-xp-surface/50 px-3 py-2"
              >
                ...
                <button
                  ...
                  className="flex h-6 w-6 shrink-0 items-center justify-center rounded-[2px] text-xp-text-secondary transition-colors hover:bg-xp-red/10 hover:text-xp-red"
