...
const AdvancedSelectDialog = ({
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[85vh] w-[600px] flex-col rounded-[2px] border border-xp-border bg-xp-surface shadow-xl"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-xp-border p-4">
          ...
          <button
            onClick={onClose}
            className="rounded-[2px] p-1 text-xp-text-muted hover:bg-xp-surface-light hover:text-xp-text"
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  ...
                  className={`flex-1 rounded-[2px] border px-3 py-2 text-sm transition-colors ${
                    selectionMode === opt.value
                      ? 'border-xp-accent bg-xp-accent text-[var(--xp-bg)]'
              ...
              >
                ...
                <div className="mb-3 flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-[2px] border border-xp-border bg-xp-bg px-3 py-2 text-sm text-xp-text placeholder-xp-text-muted focus:border-xp-blue focus:outline-none"
                  />
                  <button
                    onClick={addExtensionsFromInput}
                    disabled={!extensionInput.trim()}
                    className="rounded-[2px] bg-xp-accent px-3 py-2 text-sm text-[var(--xp-bg)] disabled:opacity-50"
                ...
                <div className="mb-3 flex flex-wrap gap-1.5">
                  {COMMON_FILE_TYPES.map((type) => (
                    <button
                      ...
                      className="rounded-[2px] border border-xp-border bg-xp-bg px-2 py-1 text-xs text-xp-text transition-colors hover:bg-xp-surface-light"
                    >
                      {type.label}
                    </button>
                  ))}
                ...
                {selectedExtensions.size > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {Array.from(selectedExtensions).map((ext) => (
                      <span
                        key={ext}
                        className="bg-xp-selection inline-flex items-center gap-1 rounded-[2px] px-2 py-0.5 text-xs text-xp-text"
                      >
                        .{ext}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min="0"
                    placeholder="0"
                    className="flex-1 rounded-[2px] border border-xp-border bg-xp-bg px-3 py-1.5 text-sm text-xp-text focus:border-xp-blue focus:outline-none"
                  />
                ...
                <div className="mb-3 grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-[2px] border border-xp-border bg-xp-bg px-3 py-1.5 text-sm text-xp-text focus:border-xp-blue focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-xp-text-muted">
              ...
              >
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 rounded-[2px] border border-xp-border bg-xp-bg px-3 py-1.5 font-mono text-sm text-xp-text placeholder-xp-text-muted focus:border-xp-blue focus:outline-none"
                  />
                  <div className="flex overflow-hidden rounded-[2px] border border-xp-border">
                    <button
                      onClick={() => setPatternMode('glob')}
                      className={`px-2 py-1.5 text-xs transition-colors ${
                        patternMode === 'glob'
                          ? 'bg-xp-accent text-[var(--xp-bg)]'
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 rounded-[2px] border px-2 py-1.5 text-xs transition-colors ${
        ...
        <div className="border-t border-xp-border p-4">
          {/* Match preview */}
          <div className="mb-3 flex items-center justify-between rounded-[2px] border border-xp-border bg-xp-bg px-3 py-2">
            <span className="text-sm text-xp-text">
              {selectionMode === 'invert'
                ? t('explorer.advancedSelect.filesAfterInvert')
                : t('explorer.advancedSelect.filesMatching')}
            </span>
            <span className="text-lg font-medium text-xp-text">
              {matchCount}{' '}
          ...
          <div className="flex justify-end gap-2">
            <button
              onClick={onClose}
              className="rounded-[2px] border border-xp-border px-4 py-2 text-sm text-xp-text-muted transition-colors hover:bg-xp-surface-light hover:text-xp-text"
            ...
            <button
              ...
              className="rounded-[2px] bg-xp-accent px-4 py-2 text-sm text-[var(--xp-bg)] transition-colors hover:bg-xp-accent-hover disabled:opacity-50"
  ...
  return (
    <div
      className={`rounded-[2px] border transition-colors ${
        enabled
          ? 'bg-xp-selection border-[var(--xp-selection-border)]'
          : 'bg-xp-bg/30 border-xp-border'
      }`}
    >
      <label className="flex cursor-pointer select-none items-center gap-2 p-3">
        <input
          ...
          className="rounded-[2px] border-xp-border accent-xp-accent"
        />
